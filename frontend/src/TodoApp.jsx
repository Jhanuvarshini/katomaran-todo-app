...
export default function TodoDashboard({ token, onLogout }) {
  ...
  const getDueLabel = (task) => {
    ...
    if (isOverdue) return <span className="text-red-600">Missed deadline</span>;
    if (isToday) return <span className="text-orange-600">Due today</span>;
    if (isTomorrow) return <span className="text-yellow-500">Due tomorrow</span>;
    return <span className="text-gray-400">Due: {due.toLocaleDateString()}</span>;
  };
            ...
            <ul className="space-y-4">
              {completedTasks.map((task) => (
                ...
                >
                  <div className="flex items-start gap-3">
                    <button onClick={() => toggleComplete(task)}>
                      <CheckCircle className="text-white bg-green-500 rounded-full p-1 shadow" size={22} />
                    </button>
                    <div>
                      <div className="line-through text-gray-500">{task.title}</div>
                      <div className="text-sm text-gray-400 mt-1">
